  ...
  useEffect(() => {
    const leftSidebar =
      ...
      document.querySelector(
        ".fixed.left-0.h-full.w-64, .fixed.left-0.h-full.w-72",
      ) ||
    ...
    <>
      ...
      <div
        ...
        className={`
          ...
          bg-gradient-to-b from-white via-[#fffdf5] to-[#fff7e6]
          shadow-2xl shadow-[#D4AF37]/25
          transform overflow-y-auto transition-all duration-400 ease-out
          ${open ? "translate-x-0 opacity-100" : "translate-x-full opacity-0 pointer-events-none"}
          flex flex-col rounded-l-2xl border-l border-[#D4AF37]/30
        `}
        role="dialog"
        aria-modal="true"
      ...
      >
        <button
          onClick={onClose}
          className="absolute top-5 right-5 text-[#8a6d1f] hover:text-[#3d3200] transition-colors duration-200 z-10"
          aria-label="Close sidebar"
        >
          <i className="fa-solid fa-xmark text-2xl" />
        </button>

        <div className="px-7 py-10 border-b border-[#D4AF37]/30 bg-gradient-to-r from-[#D4AF37]/10 to-transparent">
          <div className="flex flex-col items-center text-center gap-4">
            <div
              className="w-20 h-20 rounded-full overflow-hidden shadow-lg shadow-[#D4AF37]/40 ring-2 ring-[#D4AF37]/50 transition-all duration-300 hover:ring-[#D4AF37] hover:shadow-[#D4AF37]/60"
              style={{
                background: "linear-gradient(135deg, #ccae61 0%, #d8bc76 100%)",
              }}
            >
              {data?.profile_image ? (
                <img
            ...
            <div>
              <p className="text-xl font-bold text-[#3d3200] tracking-tight">
                {displayName}
              </p>
              <p className="text-sm text-[#8a6d1f] mt-1 opacity-90 truncate max-w-[240px]">
                {displayEmail}
              </p>
            </div>
          </div>
        ...
        {role !== "admin" && (
          <div className="px-7 py-7 border-b border-[#D4AF37]/30">
            <p className="text-xs font-semibold text-[#b8962e] uppercase mb-5 tracking-wider">
              Wallet Balance
            </p>
            <div className="flex items-center justify-between text-base text-[#3d3200] mb-2">
              <span className="font-medium">Payin Wallet</span>
              <div className="flex items-center gap-4">
                <span
                  className={`font-bold transition-all duration-300 ${PayAmountVisible ? "text-emerald-600 scale-105" : "text-[#5c4a00] blur-sm"}`}
                >
                  {PayAmountVisible ? `₹${payingAmount || "0.00"}` : "•••••••"}
                </span>
                <button
                  onClick={() => setPayAmountVisible(!PayAmountVisible)}
                  className="text-[#b8962e] hover:text-[#5c4a00] transition-colors"
                  aria-label={PayAmountVisible ? "Hide amount" : "Show amount"}
                >
                  <i
                    className={`fa-solid ${PayAmountVisible ? "fa-eye-slash" : "fa-eye"} text-lg`}
                  />
                </button>
              </div>
            </div>
        ...
        <div className="px-7 py-8 flex flex-col gap-4 flex-grow">
          {role !== "admin" && (
            <Link
              to="/profile"
              onClick={onClose}
              className="flex items-center gap-4 px-5 py-4 rounded-xl bg-white hover:bg-[#fff7e6] border border-[#D4AF37]/30 hover:border-[#D4AF37] text-[#5c4a00] hover:text-[#3d3200] transition-all duration-200 shadow-sm hover:shadow-md"
            >
              <i className="fa-solid fa-user text-[#b8962e] text-xl" />
          ...
          <button
            ...
            className="mt-auto flex items-center justify-center gap-4 px-5 py-4 rounded-xl bg-red-50 hover:bg-red-100 border border-red-200 hover:border-red-400 text-red-700 hover:text-red-800 transition-all duration-200 shadow-sm hover:shadow-md disabled:opacity-50"
          >
        ...
        <div className="px-7 py-5 text-center text-xs text-[#b8962e]/70 border-t border-[#D4AF37]/30 mt-auto">
          Soulful Nidhi Limited Dashboard • © 2026
        </div>
      </div>
        ...
        >
          ...
          >
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <h3 className="text-xl font-semibold text-[#3d3200] mb-1">
                  Confirm Sign Out
                </h3>
                <p className="text-sm text-[#8a6d1f] leading-relaxed">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                ...
                className="flex-1 py-3 px-4 bg-[#fff7e6] hover:bg-[#f5e6a3] text-[#8a6d1f] rounded-xl font-medium transition-all duration-200 border border-[#D4AF37]/30 hover:border-[#D4AF37] text-sm"
              >
                Cancel
              </button>
            </div>
