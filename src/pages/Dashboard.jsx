...
export const Dashboard = () => {
  ...
  // ─── APIs ───
  const { data: cardData, loading: recordLoading } = useAutoFetch(
    "/collection-record",
    90000,
  );

  const { data: summaryData, loading: summaryLoading } = useAutoFetch(
    "/collection-summary",
    60000,
  );

  const { data: statusCounts, loading: statusLoading } = useAutoFetch(
    "/collection-statuscounts",
    120000,
  );
  const { data: monthwiseData, loading: monthLoading } = useAutoFetch(
    "/collection-monthwise",
    120000,
  );
  const { data: cashfreeData, loading: cashfreeLoading } = useAutoFetch(
    "/collection-cashfree",
  );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-7 mb-12">
          {cardsToShow.map((card, i) => {
            ...
            return (
              <div
                key={i}
                className={`group relative bg-white rounded-tl-3xl rounded-br-3xl shadow-[0_10px_30px_rgba(212,175,55,0.15)] overflow-hidden transition-all duration-400 hover:shadow-[0_25px_60px_rgba(212,175,55,0.25)] hover:-translate-y-2 border border-[#D4AF37]/30 min-w-0`}
              >
                <div
                  className="absolute top-0 left-0 right-0 h-3 transform -skew-x-12 origin-left"
                  style={{
                    background:
                      "linear-gradient(90deg, rgb(198, 166, 83) 0%, rgb(249, 241, 214) 100%)",
                  }}
                />
                <div className="p-6 pt-9 relative">
                  <h3 className="text-sm font-semibold text-[#8a6d1f] mb-2.5 tracking-wider uppercase">
                    {card.title}
                  </h3>
                  <div
                    className="font-bold text-[#3d3200] whitespace-nowrap w-full tabular-nums text-[clamp(10px,1.5vw,20px)]"
                    title={`₹${amountStr}`}
                  >
                    ₹{amountStr}
                  </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5">
              ...
              <div className="relative">
                <select
                  ...
                  className="bg-white border border-slate-200 text-slate-700 text-sm rounded-full px-6 py-3 focus:ring-2 focus:ring-yellow-500 focus:border-blue-500 outline-none min-w-[200px] transition-all shadow-sm hover:border-blue-300 appearance-none cursor-pointer"
                >
