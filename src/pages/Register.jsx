...
const EMAIL_REGEX =
  /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*\.[a-zA-Z]{2,12}$/i;
...
const Register = () => {
  ...
  const { execute: sendEmailOtpApi, loading: emailOtpLoading } =
    usePost("/otp/send-email");
  ...
  const sendEmailOtpHandler = async () => {
    ...
    if (!isValidCompany) {
      setFieldErrors((prev) => ({
        ...prev,
        companyName: "3–100 letters only",
      }));
  ...
  const renderOtpInputs = (
    ...
    <div className="space-y-3 mt-4">
      <label className="block text-sm font-medium text-gray-700">
        Enter 6-digit code
        {timer > 0 && (
          <span className="text-red-600 ml-2">({formatTime(timer)})</span>
        )}
        {timer === 0 && expired && (
        ...
        >
          <div className="p-6 sm:p-8 lg:p-10 xl:p-12 space-y-8 lg:space-y-10">
            ...
            <div className="text-center space-y-5">
              ...
              <div className="w-full max-w-xs mx-auto mt-4">
                    ...
                    className={`
                      ...
                      ${
                        emailOtpVerified && mobileOtpVerified
                          ...
                          : emailOtpVerified
                            ? "w-2/3 bg-gradient-to-r from-[#3d3200] to-[#CCB058]"
                            : "w-1/3 bg-gradient-to-r from-[#3d3200] to-[#CCB058]"
                      }
                    `}
                    style={{
                      transition:
                ...
                <div className="flex justify-between text-xs text-gray-500 mt-2 font-medium tracking-wide">
                  <span
                    className={
                      emailOtpVerified ? "text-green-600 font-semibold" : ""
                    }
                  >
                    Ready
                  </span>
                  <span
                    className={
                      mobileOtpVerified ? "text-green-600 font-semibold" : ""
                    }
                  >
                    Email
                  </span>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <FloatingInput
                placeholder={
                  emailOtpSent ? "Check your inbox " : "Business Email"
                }
                type="email"
                  ...
                  className={`
                    ...
                    ${
                      !isValidCompany || !isValidEmail || emailOtpLoading
                        ? "bg-gray-300 text-gray-600 cursor-not-allowed"
                        : "bg-gradient-to-r from-[#b8962e] via-[#D4AF37] to-[#f5e6a3] text-[#3d3200] hover:from-blue-650 hover:to-indigo-650 hover:shadow-lg hover:shadow-blue-500/30 active:scale-[0.98]"
                    }
                  `}
                >
